...
try
{
  ...
  else if(cmds.at(0) == "signing-slave")
  {
    ...
    if (!showZone(dk, DNSName(cmds.at(1)))) return 1;
  }
    ...
    if (!showZone(dk, DNSName(cmds.at(1)), true)) return 1;
  }
    ...
    for(size_t n=1; n + 2 <= cmds.size(); n+=2)
      loadZone(DNSName(cmds.at(n)), cmds.at(n+1));
    return 0;
  }
    ...
    for(DNSBackend *b : BackendMakers().all()) {
      if (b->getPrefix() == cmds.at(1)) src = b;
      if (b->getPrefix() == cmds.at(2)) tgt = b;
    }
    if (!src) {
